function  [out_f, out_A, out_theta, out_psi] = fastMonogenic(image,rangeBase,rangeStep)

   warning off
   
   % subsample image (halve the resolution) to make FFT faster
   ...
   % upsample output images; select scale 1 for our application
   out_f = imresize(f(:,:,1),2);
   out_psi = imresize(psi(:,:,1),2);
   out_theta = imresize(theta(:,:,1),2);
   out_A = imresize(A(:,:,1),2);