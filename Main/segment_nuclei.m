...
function N = segment_nuclei(I,classes)
    ...
    % specify radius of circle to fit
%     radius = 40;
    ...
    for i = 1 : numcells
        ...
        % subsample for larger masks
        [dx dy] = size(mask);
        if dx > 200 
            x_list = [1:floor(dx/10):dx];
        else
            x_list = 1:20:dx;
        end
        
        if dy > 200 
            y_list = [1:floor(dy/10):dy];
        else
            y_list = 1:20:dy;
        end
        ...
        % for every 20 pixels in the mask
        for x = x_list
            for y = y_list
